document.addEventListener("DOMContentLoaded", () => {
  ...
  cartDrawerButtonsArray.forEach((cartDrawerButton) => {
    cartDrawerButton.addEventListener('click', () => {  
      cartDrawerButton.classList.add('shake')
      cartDrawer.toggleAttribute('open');
      if (!cartDrawer.hasAttribute('open')) {
        cartDrawer.focus();
        cartDrawerButton.classList.remove('shake')
      }
    });
  });
  ...
  const mobileSubMenuButton = document.querySelectorAll('.parent-item > div > button.open-submenu');

  mobileSubMenuButton.forEach((button)=>{
    button.addEventListener('click', ()=>{
      var submenu = button.parentNode.nextElementSibling;
      if (submenu && submenu.classList.contains('submenu-mobile-lv1')) {
          submenu.toggleAttribute('open')
          button.toggleAttribute('submenu-opened')
      }
    })
  })
